...
impl SpaceRock {
    ...
    pub fn from_spice(name: &str, epoch: &Time, frame: &CoordinateFrame, origin: &Origin) -> Self {

        let mut ep = epoch.clone();
        ep.to_utc();
        let et = spice::str2et(&format!("JD{epoch} UTC", epoch=ep.epoch));
        let (state, _) = spice::spkezr(name, et, frame.as_str(), "NONE", &origin.to_string());
    ...
    pub fn from_state(name: &str, state: StateVector, epoch: Time, frame: &CoordinateFrame, origin: &Origin) -> Self {
    ...
    pub fn from_kepler(name: &str, orbit: KeplerOrbit, epoch: Time, frame: &CoordinateFrame, origin: &Origin) -> Self {
    ...
    pub fn to_ssb(&mut self) {
        ...
        let mut ssb = SpaceRock::from_spice("ssb", &self.epoch, &self.frame, &self.origin);
    ...
    pub fn to_helio(&mut self) {
        ...
        let sun = SpaceRock::from_spice("sun", &self.epoch, &self.frame, &self.origin);
    ...
    pub fn evec(&self) -> Vector3<f64> {
        ...
        let hvec = self.hvec();
        let e = self.velocity.cross(&hvec) / self.origin.mu() - self.position / self.r();
        e
    }
