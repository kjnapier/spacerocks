...
impl Integrator for IAS15 {
    fn step(&mut self, state: &mut SimulationState, forces: &Vec<Box<dyn Force + Send + Sync>>) {
        ...
        'integration_loop: loop {
            ...
            let state_vectors = state.kernel.get_barycentric_states(&state.spice_bodies, state.epoch).unwrap();
            ...
            // use rayon to parallelize the computation of the substep positions
            // let mut substep_map: Vec<Vec<(Vector3<f64>, Vector3<f64>)>> = vec![Vec::new(); 8];
            // for substep in 1..8 {
            //     let hh = H[substep];
            //     for (body, particle) in state.spice_bodies.iter().zip(state.spice_particles.iter_mut()) {
            //         let (x, y, z, vx, vy, vz) = state.kernel.compute_state(body, &state.origin, state.particles[0].epoch + self.timestep * hh).unwrap();
            //         let position = Vector3::new(x, y, z);
            //         let velocity = Vector3::new(vx, vy, vz);
            ...
            for substep in 1..8 {
                let hh = H[substep];
                let state_vectors = state.kernel.get_barycentric_states(&state.spice_bodies, state.epoch + self.timestep * hh).unwrap();
                for (i, state_vector) in state_vectors.iter().enumerate() {
                    let (x, y, z, vx, vy, vz) = state_vector;
                    let position = Vector3::new(*x, *y, *z);
                    let velocity = Vector3::new(*vx, *vy, *vz);
